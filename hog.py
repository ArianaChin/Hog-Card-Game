...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player = abs((player_score//10)%10-player_score%10)
    opponent = abs((opponent_score//10)%10-opponent_score%10)
    if player==opponent:
        return True
    else:
        return False
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    max = 0
    index = 0
    h = make_averaged(roll_dice, num_samples)
    for i in range(1,11):
        if h(i, dice)>max:
            max = h(i, dice)
            index = i
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    if is_swap(free_bacon(opponent_score)+score, opponent_score)==True and (opponent_score>free_bacon(opponent_score)+score):
        return 0
    elif is_swap(free_bacon(opponent_score)+score, opponent_score)==True:
        return num_rolls
    else:
...
def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.
        Check score and opponent's score every turn, and return the number
        of rolls player should roll to get the highest scoring turn.
    *** YOUR DESCRIPTION HERE ***
    ...
    index = 0
    for i in range(11):
        hold = take_turn(i, opponent_score, dice=six_sided)
        swap = opponent_score-(hold+score)
        if is_swap(hold+score, opponent_score)==True and swap>max:
            max = swap
            index = i
        elif is_swap(hold+score, opponent_score)==False and hold>max:
            max = hold
            index = i
    return index
    # END PROBLEM 12
